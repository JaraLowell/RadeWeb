...
Write-Host "Fixing migration file to use CREATE TABLE IF NOT EXISTS..." -ForegroundColor Green

$MigrationFile = "Migrations\20251013163043_InitialCreateWithVisitorStats.cs"
...
# Create backup
Copy-Item $MigrationFile "$MigrationFile.backup"
Write-Host "Created backup: $MigrationFile.backup" -ForegroundColor Blue

# Apply the fix by creating a new migration file with the correct syntax
Write-Host "Applying fix..." -ForegroundColor Blue
...
Write-Host "✅ Migration file fixed!" -ForegroundColor Green
Write-Host "The migration now uses 'CREATE TABLE IF NOT EXISTS' instead of 'CREATE TABLE'" -ForegroundColor Green
Write-Host ""
Write-Host "Now run: dotnet ef database update --project RadegastWeb.csproj" -ForegroundColor Yellow
Write-Host ""
Write-Host "Your existing accounts will be preserved!" -ForegroundColor Green